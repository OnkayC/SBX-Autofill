...
for (const trigger of ['toolbar', 'inline']) {
  for (const url of [atlasSignInUrl, atlasSignInUrl.replace('b2c_1a_atoproduction_atlas_susi/oauth2/v2.0/authorize', 'B2C_1A_atoproduction_Atlas_SUSI/api/SelfAsserted/confirmed')]) {
    test(`fills Atlas sign-in with a mislabeled new-password field via ${trigger} at ${url}`, async () => {
      ...
      const dom = new JSDOM(`
        <form id="attributeVerification">
          ...
          <input id="signInName" type="text" placeholder="Enter a username">
          <label for="password">Password*</label>
          <input id="password" type="password" autocomplete="new-password" placeholder="Enter a password">
          <button>Sign In</button>
        </form>`, { url });
      makeInputsVisible(dom.window.document);
      const username = dom.window.document.querySelector('#signInName');
      const password = dom.window.document.querySelector('#password');
      const engine = new AutofillEngine(dom.window.document, rules);
      try {
        assert.equal((await engine.inspect(password)).focusedRole, 'current-password');
        const result = await engine.fill({
          trigger,
          initiator: trigger === 'inline' ? password : null,
          credential: { username: 'synthetic-user', password: 'synthetic-password' }
        });
        assert.equal(result.status, 'complete');
        assert.equal(username.value, 'synthetic-user');
...
for (const url of [
  atlasSignInUrl.replace('atlas_susi', 'atlas_su'),
  atlasSignInUrl.replace('atlas_susi', 'atlas_passwordreset'),
  'https://atlasauth.b2clogin.com/atlasauth.onmicrosoft.com/oauth2/v2.0/authorize?p=B2C_1A_ATLAS_PASSWORDRESET',
  atlasSignInUrl.replace('atlasauth.b2clogin.com', 'other.b2clogin.com')
]) {
  test(`keeps new-password protection outside the Atlas sign-in path: ${url}`, async () => {
    ...
    try {
      for (const trigger of ['toolbar', 'inline']) {
        ...
        if (trigger === 'inline') assert.deepEqual(result.reasons, ['new-password-requires-confirmation']);
      }
    } finally {
      engine.dispose();
      dom.window.close();
...
for (const fields of [
  ...
  '<input id="signInName"><input id="password" type="password" autocomplete="new-password"><input id="confirmPassword" type="password" autocomplete="new-password">'
]) {
  test(`keeps Atlas new-password protection for a non-login form: ${fields}`, async () => {
    ...
    try {
      for (const trigger of ['toolbar', 'inline']) {
        const result = await engine.fill({
          trigger,
          initiator: trigger === 'inline' ? dom.window.document.querySelector('#password') : null,
          credential: { username: 'synthetic-user', password: 'synthetic-password' }
        });
        assert.equal(result.passwordSatisfied, 0);
