...
export interface AutofillFrameInspection {
  ...
  focusedRole: 'username' | 'current-password' | 'new-password' | 'security-answer' | 'one-time-code' | 'ignored' | 'unknown';
  frameOrigin: string;
  frameUrl: string;
...
export class AutofillCoordinator {
  ...
  private async coordinateOnce(request: AutofillCoordinatorRequest): Promise<AutofillFrameResult> {
    ...
    const customFieldSatisfied = frameResults.reduce((total, result) => total + (result.customFieldSatisfied ?? 0), 0);
    ...
    const status =
      unavailableFrames === 0 && statuses.size === 1 && statuses.has('complete') && blockedFrames === 0
        ? 'complete'
        : passwordSatisfied + usernameSatisfied + customFieldSatisfied > 0
          ? 'partial'
          : blockedFrames > 0 || statuses.has('blocked')
            ? 'blocked'
            : 'no-target';
    const result = this.result(status, usernameSatisfied, passwordSatisfied, reasons);
    if (frameResults.some(frame => frame.customFieldSatisfied !== undefined)) {
      result.customFieldSatisfied = customFieldSatisfied;
    }
    return result;
  }
