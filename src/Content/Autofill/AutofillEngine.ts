...
export type AutofillFieldRole = 'username' | 'current-password' | 'new-password' | 'security-answer' | 'one-time-code' | 'ignored' | 'unknown';
...
export class AutofillEngine {
  ...
  async inspect(initiator: HTMLInputElement | null = null): Promise<AutofillInspection> {
    ...
    return {
      candidateCount: securityAnswers ? securityAnswers.length : analysis.targets.length,
      focusedRole: securityAnswers
        ? securityAnswers.some(answer => answer.field.element === initiator) ? 'security-answer' : 'unknown'
        : structurallyRecognizedUsername
        ? 'username'
  ...
  private fillSecurityAnswers(request: AutofillRequest, fields: FieldSnapshot[], rule: AutofillSiteRule): AutofillResult {
    ...
    const customFields = Object.values(request.credential.customFields ?? {}).filter((entry): entry is { key: string; value: string } => {
      if (!entry || typeof entry !== 'object') return false;
      const field = entry as { key?: unknown; value?: unknown };
      return typeof field.key === 'string' && typeof field.value === 'string' && field.value.length > 0;
    });
    let customFieldSatisfied = 0;
    const reasons: AutofillReason[] = [];
    for (const answer of answers) {
      const matches = customFields.filter(field => answer.names.includes(this.securityQuestionKey(field.key)));
      if (matches.length !== 1) {
        reasons.push(matches.length ? 'ambiguous-custom-field' : 'missing-custom-field');
        continue;
      }
      // A name shared by multiple displayed questions is ambiguous as well.
      const key = this.securityQuestionKey(matches[0].key);
      if (answers.filter(candidate => candidate.names.includes(key)).length !== 1) {
        reasons.push('ambiguous-custom-field');
        continue;
      }
      if (this.fillElement(answer.field.element, matches[0].value)) {
        customFieldSatisfied += 1;
      } else {
        reasons.push('field-rejected-value');
      }
    }
  ...
  private isNewPassword(field: FieldSnapshot): boolean {
    // Explicit site rules can correct mislabeled login fields.
    if (field.forcedRole === 'current-password') {
      return false;
    }
